...
const showDefaultValue = (value) => {
  return typeof value === "undefined" ? "" : chalk.dim(`[default: ${value}]`);
};
...
const parseNames = (names) => {
  ...
  let splitNames = names.match(/([\w\.\-]+)\s*,?\s*([\w\.\-]*)/).slice(1, 3);
...
class CAC {
  constructor() {
    ...
    this.cliUsage = `${chalk.yellow(this.pkg.name)} ${chalk.dim(
      "[options] [commands]"
    )}`;
    this.examples = [];

    this.addAliasOption("version", "v").addAliasOption("help", "h");
  ...
  showHelp() {
    ...
    const commandsTable = table(
      Object.keys(this.aliasCommands).map((command) => {
...
`;
  ...
  async runCommand(command) {
    ...
    if (!commandFn) {
      ...
      try {
        // ESM 动态导入（关键改动点）
        const module = await import(subCommand);
        // 兼容默认导出和命名导出
        commandFn = module.default || module[commandName] || module;
        // 缓存模块避免重复加载
    ...
    try {
      const input =
        command.name === "*" ? this.argv.input : this.argv.input.slice(1);
