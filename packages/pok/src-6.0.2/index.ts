...
import Command, { HelpCallback, CommandExample } from "./command.js";
import { OptionConfig } from "./option.js";
import { getMinimostOptions } from "./utils.js";
...
class CAC extends EventEmitter {
  ...
  parse(argv = process.argv): ParsedArgv {
    ...
    const globalMinimostOptions = getMinimostOptions(
      this.globalCommand.options
    );
    const parsed = this.minimost(argv.slice(2), globalMinimostOptions);
    this.args = parsed.args;
    this.options = parsed.options;
    // console.log("🚀 ~ CAC ~ parse ~ parsed.options:", parsed.options)

    /** 全局命令注册了 help 选项, 且当前用户输入了 help 选项 */
  ...
  minimost(argv: string[], minimostOptions: MinimostOpts) {
    const { input: args, flags: options } = (minimost as any)(
      argv,
      minimostOptions
    );
